...
# v pripade opaku je tedy NUTNE vytvorit ciste z tohoto duvodu napr. prikazem touch aaa/smazat1.txt aaa/smazat2.txt aaa/smazat3.># a prikazem "unzip -l aaa_v6.2.zip | head -n 11" potom jeste radeji zkontrolovat vsechny informace

cesta_soubory_hesel="/data/data/com.termux/files/home/work/verze_6.2_bash/keys/"
...
echo "pocet poly v souboru "$1" = "$matrix_svisle

heslo=""
zip_file_comment=""
for (( aa = 0; aa <= $matrix_svisle - 1; aa++ ));do
...
# heslo+=${n_X[$rnd]} < puvodni verze 6.1
heslo+=${matrix[$aa,$rnd]} # novy prirazeni pro verzi z matrix ( bez aval )
#cmd_1='heslo+=${n_'
#cmd_1+=$aa
#cmd_1+='[$rnd]}'
##echo $cmd_1
...
archiv_zip_nazev=${zip_folder:0:${#zip_folder}-1}"_v6.2.zip"
