...
echo "matrix_vodorovne="$matrix_vodorovne > $cesta_soubory_hesel$1
echo "matrix_svisle="$((matrix_svisle + 1 )) >> $cesta_soubory_hesel$1
echo "matrix_delka="$(($matrix_vodorovne * $(( matrix_svisle + 1 )) )) >> $cesta_soubory_hesel$1

echo "declare -A matrix=(" >> $cesta_soubory_hesel$1

for (( aa = 0; aa <= matrix_svisle; aa++ )) do
echo "# "$aa",10-"$((matrix_vodorovne + 10 ))
echo "# "$aa",10-"$((matrix_vodorovne + 10 )) >> $cesta_soubory_hesel$1
...
echo $prvek
echo $prvek >> $cesta_soubory_hesel$1
#out_1+=$prvek
done
done

echo ")" >> $cesta_soubory_hesel$1
